...
function preload() {
  font = loadFont('/APOLLOItalic-subset.otf');
}
...
function setup() {
  ...
  pixelDensity(1);
  textFont(font);
  colorArray = generateColorGradient(colors.accent, colors.background, params.numberOfCurves);
}
...
function createNoisyCircles(centerX, centerY, colorArray) {
  for (j = 0; j < params.numberOfCurves; j++) {
    ...
    offsetX = n * cos(circlePoint) * curveNoiseFactor * curveRadius / (params.numberOfCurves * params.gap);
    offsetY = n * sin(circlePoint) * curveNoiseFactor * curveRadius / (params.numberOfCurves * params.gap);
